...
function getArtistByIndex(array,index) {
  let nameOfIndex = array[index];
  let name = nameOfIndex.name;
return `the artist at index ${nameOfIndex.id} is ${nameOfIndex.name}`; 
}
...
function get20s(array) {            
  ...
  let diedIn20Th = [];
    ...
    startYear = array[i].years.split(" ")[0];
    startYear = parseInt(startYear);
    endYear = array[i].years.split(" ")[2];
    endYear = parseInt(endYear);
    if(startYear > 1900 && endYear < 2000){
      diedIn20Th.push(array[i].name);
    }
  } return diedIn20Th;
}
get20s(artists);
...
console.log("LOOK AT M!", artists[1].paintings)
